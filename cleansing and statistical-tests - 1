...
ggplot(plot.data, aes(x=X1, y=prob, color=Group)) + # asking it to set the color by the variable "group" is what makes it draw three different lines
  geom_line(lwd=2) + 
  labs(x="BMI (Kg/m^2)", y="Mortality rate (%)", title="How BMI size affects hospitalized mortality in the immediate term",
       subtitle = "Divided by gender and age groups",
       caption = "Intervals between dashed lines represent\nBMI groups that differ in mortality rates:\n15-18.5, 18.5-25 and 25-40") +
  ...
  theme(plot.title = element_text(face="bold",size = 14), axis.title.x = element_text(hjust=0.5,size = 14),axis.title.y = element_text(hjust=0.5,size = 14),
        ...
        plot.subtitle = element_text(, size = 13), legend.title = element_text(size=14), legend.text = element_text(size=13))
...
# all of the patients died after age of 20
main_df = obesity_albumin_1%>% filter(age_during_hospitalization >=20)

linear_reg() %>%
  set_engine("lm") %>%
  fit(albumin ~ age_during_hospitalization, data = main_df) %>%
  tidy()
